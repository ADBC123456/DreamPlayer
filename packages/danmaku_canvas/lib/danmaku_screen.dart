...
class _DanmakuScreenState extends State<DanmakuScreen>
  ...
  bool addDanmaku(
    ...
    if (content.type == DanmakuItemType.special) {
      ...
      final luminance = (0.299 * (color.r * 255.0).round() +
    ...
    final remainingDuration = (_option.duration - elapsedSeconds)
        .clamp(0.0, _option.duration)
        .toDouble();
  ...
  bool _advanceStaticLifetimes(int tick, double playbackRate) {
    ...
    for (final items in [_topDanmakuItems, _bottomDanmakuItems]) {
      ...
      items.removeWhere(
          (item) => (item.remainingDurationSeconds ?? _option.duration) <= 0);
