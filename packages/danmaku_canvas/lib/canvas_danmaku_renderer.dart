...
class _CanvasDanmakuRendererState extends State<CanvasDanmakuRenderer> {
  ...
  final Map<Map<String, dynamic>, int> _restorationRetryCounts = Map.identity();
  ...
  void _processAndAddDanmaku(
    ...
    final timeDiff =
        hasPreviousTime ? (currentTime - _lastCurrentTime).abs() : 0.0;
    ...
    final listReplaced =
        _lastDanmakuList != null && !identical(_lastDanmakuList, danmakuList);
    final fallbackSeek = widget.seekRevision < 0 &&
        hasPreviousTime &&
        timeDiff > CanvasDanmakuTimeline.fallbackSeekThresholdSeconds;
    final shouldRestore =
        _forceTimelineRestore || listReplaced || entriesRemoved || fallbackSeek;
  ...
  void _processNormalWindow(
    ...
    for (int i = startIndex; i < danmakuList.length; i++) {
      ...
      if (_tryAddDanmaku(danmakuData, currentTime, isRestoration: false)) {
        addedCount++;
